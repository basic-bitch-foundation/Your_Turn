...
func do_cut(whl):
	...
	# disable collision so knife doesn't interact with cut apple
	var col = get_node_or_null("CollisionShape2D")
	if col:
	...
	# after animation, just HIDE — never queue_free
	# the node stays alive so reset_apple can bring it back
	await get_tree().create_timer(1.8).timeout
	if is_instance_valid(self) and been_cut:
		flying  = false
		visible = false
