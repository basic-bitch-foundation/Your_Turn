...
func _process(delta):
	if going:
		# save position BEFORE moving — needed for correct approach direction
		var old_g = global_position

		global_position.y -= spd * delta

		var dist = global_position.distance_to(log_node.global_position)
		if dist <= log_r:
			# pass old_g so arrive_at_log knows which side we came from
			# without this, fast knives overshoot center and clamp to wrong side
			arrive_at_log(old_g)
			return

		if global_position.y < miss_y and not hit_something:
			going = false
			set_process(false)
...
func arrive_at_log(old_g: Vector2):
	# use PRE-MOVE position to get the true approach direction
	# if we used current global_position, an overshooting knife would
	# compute the wrong direction (pointing up instead of down)
	# and clamp to the top of the log instead of the bottom
	var from_ctr = old_g - log_node.global_position
	if from_ctr.length() < 0.001:
		from_ctr = Vector2.DOWN
	var cdir = from_ctr.normalized()

	# snap to log surface on the correct side
	global_position = log_node.global_position + cdir * log_r

	var cang = cdir.angle()

	# 1. stuck knife at this angle → clash
