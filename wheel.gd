...
var origin: Vector2
var shake = 0.0
var active = true   # spins from start
...
func _process(delta):
	if active:
		rotation += rot_spd * float(dir) * delta
...
func do_bounce():
	var t = create_tween()
	t.tween_property(self, "position", origin + Vector2(0.0, -16.0), 0.07).set_trans(Tween.TRANS_SINE)
